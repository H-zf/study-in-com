...
## H5页面手机上联调
手机和电脑同时链接同一个wifi
（本地起服务，然后手机和电脑用同一个wifi 右键chrome创建二维码的页面 进行手机扫描看 可以用wif连接的wifi来替换项目启动的localhost）
...
2。后端使用cors跨域的时候必须要设置允许跨域的域 必须指明是哪个域名 单纯的设置*是不可以的 会报错
...
（跳转链接分成两种，一种是scheme协议链接，同一个scheme标识只能标识一个用户，另外一个用户标识不了，所以不同用户扫码每次都要重新生成；
一种是短链，由后台请求接口得到，是https类似的链接，可以长期使用，并且每个用户都可以进去，不用每次生成）
